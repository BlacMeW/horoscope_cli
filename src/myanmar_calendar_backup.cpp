...
void MyanmarCalendar::initializeExceptionData() {
    ...
    // Watat exceptions - these are now handled in getMyanmarConstants()
    // The specific exceptions are era-dependent and included there
    watatExceptions = {
        // These are kept for backward compatibility but the logic is now in getMyanmarConstants()
    };
}
...
void MyanmarCalendar::getMyanmarConstants(long myanmarYear, double& eraId, double& watatOffset,
    ...
    if (myanmarYear >= 1312) {
        ...
        watatOffset = -0.5;
        monthNumber = 8.0;
        // Full moon day offset exceptions for era 3
        if (myanmarYear == 1377) watatOffset += 1.0;
        // Watat exceptions for era 3
        if (myanmarYear == 1344 || myanmarYear == 1345) {
            exceptionWatat = (myanmarYear == 1344) ? 1 : -1;
        }
    }
    // The second era (the era under British colony: 1217 ME - 1311 ME)
        ...
        watatOffset = -1.0;
        monthNumber = 4.0;
        // Full moon day offset exceptions for era 2
        if (myanmarYear == 1234) watatOffset += 1.0;
        else if (myanmarYear == 1261) watatOffset -= 1.0;
        // Watat exceptions for era 2
        if (myanmarYear == 1263 || myanmarYear == 1264) {
            exceptionWatat = 1;
        }
    }
    ...
    // Thandeikta (ME 1100 - 1216)
        ...
        std::vector<std::pair<long, int>> fme = {{1120, 1}, {1126, -1}, {1150, 1}, {1172, -1}, {1207, 1}};
        for (const auto& exception : fme) {
            if (myanmarYear == exception.first) {
                watatOffset += exception.second;
                break;
            }
        }
        // Watat exceptions
        if (myanmarYear == 1201 || myanmarYear == 1202) {
            exceptionWatat = 1;
        }
    }
    // Makaranta system 2 (ME 798 - 1099)
        ...
        monthNumber = -1.0;
        // Full moon day offset exceptions
        std::vector<std::pair<long, int>> fme = {
...
void MyanmarCalendar::calculateWatat(long myanmarYear, long& watat, long& fullMoonDay) const {
    ...
    if (eraId >= 2.0) {
        ...
        double thresholdWatat = LUNAR_MONTH - (SOLAR_YEAR / 12.0 - LUNAR_MONTH) * monthNumber;
        if (excessDay >= thresholdWatat) watat = 1;
    } else {
...
void MyanmarCalendar::julianToMyanmarDate(double julianDay, long& yearType, long& myanmarYear,
    ...
    // More accurate month calculation following reference implementation
    ...
    // Handle first Waso (month 0) and ensure proper month numbering
    if (month <= 0) month = 4; // First Waso becomes regular Waso
}
...
MyanmarCalendarData MyanmarCalendar::calculateMyanmarDate(double julianDay) const {
    ...
    // Calculate weekday (Saturday = 0, Sunday = 1, ..., Friday = 6)
    // Standard Julian Day weekday: (JD + 1) % 7 gives Monday=0, but we want Saturday=0
    long weekday = (long(julianDay) + 2) % 7; // Correct weekday calculation for Myanmar system
...
MyanmarYearType MyanmarCalendar::getYearType(long myanmarYear) const {
    ...
    switch (yearType) {
        case 0: return MyanmarYearType::COMMON;
        case 1: return MyanmarYearType::LITTLE_WATAT;
        case 2: return MyanmarYearType::BIG_WATAT;
        default: return MyanmarYearType::COMMON;
    }
}
