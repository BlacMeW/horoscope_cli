...
void testSolarTerms() {
    ...
    SolarTerm lastTerm = SolarTerm::DAHAN;  // Should be the last term of previous year
    
    for (int days = 0; days < 365; days += 15) {  // Check every 15 days
        double jd = startJD + days;
        SolarTerm currentTerm = calendar.getCurrentSolarTerm(jd);
        
        if (days < 100) {  // Only print first few
            std::cout << "Day " << days << ": " << calendar.getSolarTermName(currentTerm, false) << "\n";
        }
        
        // Terms should generally progress (with year boundary exception)
        if (days > 0 && currentTerm != lastTerm) {
